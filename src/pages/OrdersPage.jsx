...
function CountdownTimer({ expira_en = "" }) {
  CountdownTimer.propTypes = {
    expira_en: PropTypes.string,
  };
  const [timeLeft, setTimeLeft] = useState(null);

  useEffect(() => {
    const calculate = () => {
      const diff = new Date(expira_en) - new Date();
      if (diff > 0) {
        setTimeLeft({
...
export default function OrdersPage() {
  ...
  const generarLinkPago = async (pedido) => {
    try {
      ...
      payloadCart = payloadCart.filter((item) => {
        ...
        return item.nombre && cantidad > 0 && precio > 0;
      });

      const totalCarrito = payloadCart.reduce(
        (a, b) => a + Number(b.precio) * Number(b.cantidad), 0
      );
      const diferencia = Math.round(Number(pedido.total) - totalCarrito);
      if (diferencia > 0) {
        payloadCart.push({
      ...
      if (!response.ok) throw new Error("Error al generar pago");
      ...
      console.error(error)
      showError("Error al generar link de pago");
    }
  };
            ...
            {pedidosFiltrados.map((pedido) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                    {/* Payment Section */}
                    {pedido.estado === "configurado" && pedido.metodo_pago === "mercadopago" && (
                      <>
                        {pedido.expira_en && !tiempoExpirado && (
                          <div className="mb-4 p-4 bg-blue-50 rounded-xl border border-blue-200">
                            ...
                            <button
                              onClick={() => generarLinkPago(pedido)}
                              className="w-full bg-[#009EE3] hover:bg-[#007DB5] text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2"
                            >
                              <CreditCard size={18} />
