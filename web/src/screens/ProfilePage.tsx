...
export default function ProfilePage() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      const data = await fetchPerfilByEmail(userProfile.email);
      setProfileData(data);
      setEditForm({
        nombre: data.nombre || "",
  ...
  const loadRecentOrders = useCallback(async () => {
    if (!user?.id) return;
    try {
      const data = await fetchPedidosByUserId(user.id);
      setPedidosRecientes(Array.isArray(data) ? data.slice(0, 5) : []);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updatePerfil(profileData.id, {
