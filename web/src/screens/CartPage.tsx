...
export default function CartPage() {
  ...
  useEffect(() => {
    getShippingPriceFromDB().then(setShippingPrice);
  }, []);
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      try {
        ...
        const data = await fetchPerfilByEmail(user.email);
        if (!data) return;
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          <div className="flex items-center border-2 border-gray-200 rounded-lg overflow-hidden">
                            <button
                              ...
                              className="w-9 h-9 sm:w-10 sm:h-10 font-bold bg-gray-50 hover:bg-gray-200 text-sm sm:text-base"
                            >
                              -
                            </button>
                            <input
          ...
          {cart.length > 0 && (
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-4">
                <h2 className="text-2xl font-black mb-6">Finalizar Compra</h2>

                {/* Banner de datos verificados si está logueado */}
                {user && (
                  <div className="mb-6 flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm font-bold">
                    <Lock size={16} />
                ...
                <button
                  ...
                  className="w-full mt-6 bg-[#FF6600] hover:bg-orange-700 text-white font-black rounded-xl py-4 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isSubmitting ? (
                    <>
